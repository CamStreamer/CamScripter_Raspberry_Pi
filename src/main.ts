...
import { ResponseCode, sendMessageResponse, sendJsonResponse, sendParamResponse, logger, Paths, getVersion } from './commonData';
...
http_server.on('filerequest', (raw_url: string, res: ServerResponse) => {
    if (raw_url.match(/package/)) {
        ...
        let file_path = raw_url in aliases ? './html/' + path.normalize(aliases[raw_url]) : './html/' + path.normalize(raw_url);
        let parsed = path.parse(file_path);
...
http_server.on('proxy', (rest: string, req: IncomingMessage, res: ServerResponse, proxy_mirror: http_proxy, is_public: boolean) => {
    ...
    if (items.length < 2) {
        ...
        if (package_name in pckg_manager.packages) {
            ...
            if (is_public) {
                target_port = pckg_manager.packages[package_name].env_vars.http_socket_public;
            } else {
                target_port = pckg_manager.packages[package_name].env_vars.http_socket;
            }
            req.url = '/' + rest;
            proxy_mirror.web(req, res, { target: 'http://localhost:' + target_port });
        } else {
...
http_server.registerRequestCGI('/package/remove.cgi', (url, res) => {
    let pckg_name = url.searchParams.get('package_name');
    if (!pckg_name) {
        sendJsonResponse(res, ResponseCode.BAD_REQ, { 'message': 'No name provided!' });
    } else if (pckg_manager.contains(pckg_name)) {
